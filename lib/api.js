...
class Philips extends EventEmitter {
    ...
    async startNotifychange() {
        ...
        const nettvversion = await this.requests(`${this.https}/system/nettvversion`, null, this.pw, this.user, "POST");
        const channeldb = await this.requests(`${this.https}/channeldb/tv`, null, this.pw, this.user, "POST");
        const activities_current = await this.requests(
            `${this.https}/activities/current`,
            null,
            this.pw,
            this.user,
            "POST",
        );
        const activities_tv = await this.requests(`${this.https}/activities/tv`, null, this.pw, this.user, "GET");
        const network = await this.requests(`${this.https}/network/devices`, null, this.pw, this.user, "GET");
        const powerstate = await this.requests(`${this.https}/powerstate`, null, this.pw, this.user, "GET");
        const applications = await this.requests(`${this.https}/applications`, null, this.pw, this.user, "GET");
        ...
        this.notify = {
            notification: {
                ...
                "activities/tv": activities_tv,
                "channeldb/tv": channeldb,
                "menuitems/settings/version": {
                    version: time_now,
                    nodes: this.req_id,
                },
                "system/nettvversion": nettvversion.nettvversion ? nettvversion.nettvversion : "NA",
                powerstate: powerstate,
                "applications/version": applications.version != null ? applications.version : "NA",
                "activities/current": activities_current,
                "recordings/list": { version: time_now },
        ...
        this.notifyInterval = this.adapter.setInterval(() => {
            this.adapter.log.info(`NOTIFY: ${JSON.stringify(this.notify)}`);
            this.requestClient
                ...
                .then((res) => {
                    ...
                    if (typeof res.data === "object") {
                        for (const key in res.data) {
                            if (this.notify.notification && this.notify.notification[key]) {
                                if (key == "aurora/settings/value" || key == "aurora/settings/gallery_upgrade") {
