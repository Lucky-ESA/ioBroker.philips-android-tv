...
class Philips extends EventEmitter {
    ...
    setStatus(val) {
        this.setValue(`${this.dp}.status.online`, val);
    ...
    setStateCached(cached) {
        if (cached != null && cached.layer1 != null) {
            this.setValue(`${this.dp}.remote.settings.ambilight_hex`, JSON.stringify(cached));
            this.color = cached;
        }
    }
    ...
    async sendRequest(state, set, fav = null) {
        ...
        if (set === "vol" && this.volume && this.volume.muted != null) {
            ...
            methode = "PUT";
            send = state;
            html = `/channeldb/tv/favoriteLists/${fav}`;
        }
        if (send != null) {
            this.adapter.log.debug(`SEND: ${JSON.stringify(send)} - METHODE: ${methode}`);
    ...
    async sendColor(hex, set, site) {
        ...
        if (send != null) {
            ...
            await this.requests(
                `${this.https}/ambilight/mode`,
                JSON.stringify({ current: "expert" }),
                this.pw,
                this.user,
                "POST",
            );
        ...
        if (data && data.context) {
            if (data.context.level1 == "WatchExtension") {
                if (data.context.level2 == "Playstate") {
                    this.setValue(`${this.dp}.status.input`, "HDMI");
                } else if (data.context.level2 == "Options") {
                    this.setValue(`${this.dp}.status.input`, "HDMI OPTIONS");
                } else {
                    this.sendUnknownContext(data.context);
                }
            } else if (data.context.level2 == "Setup_Menu") {
                if (data.context.level3 == "aurora_options") {
                    this.setValue(`${this.dp}.status.input`, "AURORA");
                } else if (data.context.level3 == "sunrise_alarm") {
                    this.setValue(`${this.dp}.status.input`, "SUNRISE ALARM");
                } else {
                    this.sendUnknownContext(data.context);
                }
            } else if (data.context.level2 == "Home") {
                if (data.context.level3 == "source_section") {
                    this.setValue(`${this.dp}.status.input`, "Settings");
                } else {
                    this.sendUnknownContext(data.context);
                }
            } else if (data.context.level1 == "WatchTv") {
                if (data.context.level2 == "Playstate") {
                    this.setValue(`${this.dp}.status.input`, "TV");
                } else if (data.context.level2 == "Options" && data.context.level3 == "channel_info") {
                    this.setValue(`${this.dp}.status.input`, "TV CHANNEL INFO");
                } else if (data.context.level2 == "Options") {
                    this.setValue(`${this.dp}.status.input`, "TV CHANNEL OPTIONS");
                } else {
                    this.sendUnknownContext(data.context);
                }
            } else if (data.context.level1 == "WatchSatellite" && data.context.level2 == "Playstate") {
                if (data.context.level2 == "Playstate") {
                    this.setValue(`${this.dp}.status.input`, "SATELLITE");
                } else if (data.context.level2 == "Options" && data.context.level3 == "channel_info") {
                    this.setValue(`${this.dp}.status.input`, "SAT CHANNEL INFO");
                } else {
                    this.sendUnknownContext(data.context);
                }
            } else if (data.context.level1 == "EPG") {
                this.setValue(`${this.dp}.status.input`, "EPG");
                ...
                if (data.context.level2 == "Playstate" && data.context["data"] == "DMR Playing") {
                    this.setValue(`${this.dp}.status.input`, "NETWORK PLAYING");
                } else if (data.context.level2 == "Playstate" && data.context.data == "DMR Loading") {
                    this.setValue(`${this.dp}.status.input`, "NETWORK LOADING");
                } else if (data.context.level2 == "Playstate") {
                    this.setValue(`${this.dp}.status.input`, "NETWORK");
                } else if (data.context.level2 == "Browsestate") {
                    this.setValue(`${this.dp}.status.input`, "NETWORK BROWSE");
                } else {
                    this.sendUnknownContext(data.context);
                }
            } else if (data.context.level1 == "BrowseUsb") {
                if (data.context.level2 == "Favourites") {
                    this.setValue(`${this.dp}.status.input`, "USB FAVORITE");
                } else if (data.context.level2 == "Browsestate") {
                    this.setValue(`${this.dp}.status.input`, "USB");
                } else if (data.context.level2 == "last_played") {
                    this.setValue(`${this.dp}.status.input`, "USB LAST PLAYED");
                } else if (data.context.level2 == "most_popular") {
                    this.setValue(`${this.dp}.status.input`, "USB MOST POPULAR");
                } else {
                    this.sendUnknownContext(data.context);
                }
            } else if (
                ...
                this.setValue(`${this.dp}.status.input`, "LAUNCH APP");
            } else {
                this.sendUnknownContext(data.context);
            }
        }
        if (data) {
            this.setValue(`${this.dp}.status.notify`, JSON.stringify(data));
        } else {
            this.setValue(`${this.dp}.status.notify`, JSON.stringify({ data: "no data" }));
        }
    }

    sendUnknownContext(data) {
        this.adapter.log.info(`Unknown context - ${JSON.stringify(data)}`);
    }
